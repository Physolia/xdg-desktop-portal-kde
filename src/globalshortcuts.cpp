                                          ...
                                          QVariantMap &results)
{
    ...
    connect(session, &GlobalShortcutsSession::shortcurtsChanged, this, [this, session, session_handle] {
...
QVariantMap GlobalShortcutsPortal::ListShortcuts(const QDBusObjectPath &handle, const QDBusObjectPath &session_handle)
{
    qCDebug(XdgDesktopPortalKdeGlobalShortcuts) << "ListShortcuts called with parameters:";
    qCDebug(XdgDesktopPortalKdeGlobalShortcuts) << "    handle: " << handle.path();
    qCDebug(XdgDesktopPortalKdeGlobalShortcuts) << "    session_handle: " << session_handle.path();

    GlobalShortcutsSession *session = qobject_cast<GlobalShortcutsSession *>(Session::getSession(session_handle.path()));
    return {
        {"shortcuts", session->shortcutDescriptionsVariant()},
    };
}

QVariantMap GlobalShortcutsPortal::BindShortcuts(const QDBusObjectPath &handle,
                                                 const QDBusObjectPath &session_handle,
